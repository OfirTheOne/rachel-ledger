  ...
  return (
    <>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
          ...
          <input
            ...
            style={{
              ...
              color: "var(--color-accent-contrast)", fontSize: "clamp(40px, 13vw, 52px)", fontWeight: 600,
      ...
      <Card delay={1} style={{ display: "grid", gap: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 12 }}>
          ...
          <Field label={t("add.note")}>
            <input style={inputStyle} placeholder={t("add.notePlaceholder")} value={note} onChange={(e) => setNote(e.target.value)} />
          </Field>
        </div>
            ...
            {categories.map((c) => {
              ...
              return (
                <button
                  ...
                  style={{
                    cursor: "pointer", padding: "8px 14px", borderRadius: 999, fontSize: 14,
                    fontWeight: active ? 600 : 500,
                    color: active ? "var(--color-accent-contrast)" : "var(--color-text)",
                    background: active ? "linear-gradient(145deg, var(--color-accent), var(--color-accent-2))" : "var(--color-surface-2)",
                    border: `1px solid ${active ? "transparent" : "var(--color-border)"}`,
                    boxShadow: active ? "var(--shadow-sm)" : "none", transition: "all 0.18s ease",
                  }}
                >
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 4, padding: 4, borderRadius: 14, background: "var(--color-surface-2)", border: "1px solid var(--color-border)" }}>
            {METHOD_VALUES.map((value) => {
              ...
              return (
                <button
                  ...
                  style={{
                    flex: 1, cursor: "pointer", padding: "9px 4px", borderRadius: 10, border: 0, fontSize: 13,
                    fontWeight: active ? 600 : 500, color: active ? "var(--color-text)" : "var(--color-text-muted)",
      ...
      <button
        ...
        className="rise"
        style={{
          width: "100%", cursor: canSave && !saving ? "pointer" : "default", padding: "16px", border: 0,
          borderRadius: "var(--radius)", fontSize: 16, fontWeight: 600, color: "var(--color-accent-contrast)",
          background: "linear-gradient(145deg, var(--color-accent), var(--color-accent-2))",
          boxShadow: "var(--shadow-lg)", opacity: canSave && !saving ? 1 : 0.55, animationDelay: "0.18s",
